...
def permByteList(inputByteList,permTable):
    outputByteList = (len(permTable) >> 3) * [0]
    for index,elem in enumerate(permTable):
        i = index % 8
        e = (elem - 1) % 8
        if i >= e:
            outputByteList[index >> 3] |= \
                (inputByteList[(elem - 1) >> 3] & (128 >> e)) >> (i - e)
        else:
            outputByteList[index >> 3] |= \
                (inputByteList[(elem - 1) >> 3] & (128 >> e)) << (e - i)
...
def encryptBlock(inputBlock):
    ...
    for round in range(16):
        expRightPart = permByteList(rightPart, EP)
        ...
        for nBox in range(4):
            ...
            SOutput[nBox] = (S[nBox << 1][leftIndex] << 4) + \
                                S[(nBox << 1) + 1][rightIndex]

        aux = permByteList(SOutput, PF)
        newRightPart = [i^j for i,j in zip(aux, leftPart)]
        leftPart = rightPart
        rightPart = newRightPart
    return permByteList(rightPart + leftPart,FP)
...
def decryptBlock(inputBlock):
    ...
    for round in range(16):
        ...
        for nBox in range(4):
            ...
            leftIndex = getIndex(indexList[nBox12:nBox12 + 6])
            rightIndex = getIndex(indexList[nBox12 + 6:nBox12 + 12])
            SOutput[nBox] = (S[nBox * 2][leftIndex] << 4) + \
                                S[nBox * 2 + 1][rightIndex]
...
def encrypt(key, inString):
    setKey(key)
    inputByteList, outputByteList = padData(inString), []
    for i in range(0, len(inputByteList), 8):
        outputByteList += encryptBlock(inputByteList[i:i + 8])
    return outputByteList
 
def decrypt(key, inputByteList):
    setKey(key)
    outputByteList = []
    for i in range(0,len(inputByteList),8):
        outputByteList += decryptBlock(inputByteList[i:i + 8])
    return unpadData(outputByteList)

def main():
    # Get input from user
    plaintext = input("Enter the text (multiple of 8 bytes): ")
    key = input("Enter a 8-byte key: ").strip()

    # Check if the key is 8 bytes
    if len(key) != 8:
        print("Error: The key must be exactly 8 bytes long.")
        return

    # Pad the input text to make it a multiple of 8 bytes
    padded_data = padData(plaintext)

    # Generate subkeys from the input key
    setKey([ord(c) for c in key])
    ...
    print("Encrypted Data (hex):", encryptedBlocks)
    
    decryptedBlocks = []
    for i in range(0, len(encryptedBlocks), 8):
        block = encryptedBlocks[i:i + 8]
        decryptedBlock = decryptBlock(block)
        decryptedBlocks.extend(decryptedBlock)

    unpaddedData = unpadData(decryptedBlocks)
    print("Decrypted Data:", unpaddedData)
